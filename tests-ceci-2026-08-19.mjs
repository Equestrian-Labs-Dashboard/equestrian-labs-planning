...
assert.match(app, /if \(idx <= 0\) return 0;\s*const priorYear = years\[idx - 1\];\s*return baseEcommerceRevenue\(priorYear\) \* organicGrowthPct\(priorYear\);/s, 'Organic Growth must start in 2027 using prior-year assumption/base');
...
assert.match(index, /assets\/js\/app\.js\?v=2151/, 'Index must bust cache for integrated app.js');
assert.equal(assumptions.meta.version, '2.15.1', 'Assumptions version must match integrated model');
...
console.log('PASS cache-busted integrated app.js v2.15.1');
