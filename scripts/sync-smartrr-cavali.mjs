...
function family(o){const t=txt(o);if(/premier|premium/.test(t))return"premier";if(/signature|welcome box|dedicated equestrian|cavali club membership/.test(t))return"signature";return"other"}
...
async function main(){const previous=await readJson(OUT,{}),shopify=await readJson(SHOPIFY,{brands:{}});if(!KEY){console.warn("No Smartrr key; preserving current file.");return}
 ...
 function tier(t){const c=weighted(g[t].active),sf=shopifyFallback(shopify,t,"annualizedBoxesPerMember");return{active:classified?g[t].active.length:null,paused:classified?g[t].paused.length:null,cancelled:classified?g[t].cancelled.length:null,boxesPerMemberYear:sf??c??previous?.[t]?.boxesPerMemberYear??null,boxesSource:sf?"shopify_ytd_annualized":c?"smartrr_plan_cadence":"previous_or_unavailable"}}
