...
function saveNow() {
  ...
  saveScenarioInputs((STATE.meta && STATE.meta.modelStatus) || "Draft");
...
function makeEditableCell(rowObj, key, onChange, opts = {}) {
  ...
  input.addEventListener("input", commitEditableValue);
  input.addEventListener("change", commitEditableValue);
...
function forecastMetric(rows, driver, year, fallbackCurrent = true) {
  const row = getRow(rows, driver);
  const direct = row && row[year];
  if (!isBlankLike(direct)) return direct;
  if (year !== "current" && !isBlankLike(row.y2026)) return row.y2026;
  if (fallbackCurrent && !isBlankLike(row.current)) return row.current;
  return "";
}
...
function baseEcommerceRevenue(year) {
  ...
  for (let i = 1; i <= idx; i++) {
    ...
    const priorOrganic = organicGrowthRevenue(priorYear);
...
function ecommerceBuild(year) {
  ...
  adSpend = totalAdSpendByYear(year);
  paid = adSpend * roas;
...
function engineGrossAndGp(engine, year) {
  ...
  if (title.startsWith("Ecommerce")) {
    ...
    const sigMembers = parseNumber(forecastMetric(rows, "Signature Active Members", year));
    const sigBoxes = parseNumber(forecastMetric(rows, "Signature Boxes per Year", year));
    const sigPrice = parseMoney(forecastMetric(rows, "Signature Price", year));
    const premMembers = parseNumber(forecastMetric(rows, "Premium Active Members", year));
    const premBoxes = parseNumber(forecastMetric(rows, "Premium Boxes per Year", year));
...
function renderFinancialSummary() {
  ...
  // KPI cards must always follow the selected Display Year.
  const selectedYearKey = `y${String((STATE.meta && STATE.meta.displayYear) || "2026")}`;
  const selectedYearLabel = yearLabel(selectedYearKey);
  const b = bridges[selectedYearKey] || bridges.y2026;
  const o = pnlOpexForYear(selectedYearKey, b);
  const ebitda = b.gp3 - o.total;
  kpiWrap.innerHTML = "";
  [
    { label: "Gross Sales", value: formatFinancialMoney(b.grossSales, {dashZero:true}), sub: `${selectedYearLabel} Forecast` },
    { label: "Net Sales", value: formatFinancialMoney(b.netSales, {dashZero:true}), sub: `Forecast ${selectedYearLabel} · After Discounts & Returns` },
    { label: "GP1", value: formatFinancialMoney(b.gp1, {dashZero:true}), sub: b.gp1 ? `Forecast ${selectedYearLabel} · ${formatPercent(b.netSales ? b.gp1 / b.netSales : 0)} of Net Sales` : `Forecast ${selectedYearLabel} · After COGS` },
    { label: "GP2", value: formatFinancialMoney(b.gp2, {dashZero:true}), sub: b.gp2 ? `Forecast ${selectedYearLabel} · ${formatPercent(b.netSales ? b.gp2 / b.netSales : 0)} of Net Sales` : `Forecast ${selectedYearLabel} · After Fulfillment` },
    { label: "GP3", value: formatFinancialMoney(b.gp3, {dashZero:true}), sub: b.gp3 ? `Forecast ${selectedYearLabel} · ${formatPercent(b.netSales ? b.gp3 / b.netSales : 0)} of Net Sales` : `Forecast ${selectedYearLabel} · After Advertising` },
    { label: "EBITDA", value: formatFinancialMoney(ebitda, {dashZero:true}), sub: `Forecast ${selectedYearLabel} · After Operating Expenses` }
  ...
  opsWrap.innerHTML = "";
  [
    { label: "Orders", value: Math.round(ordersForYear(selectedYearKey) || 0).toLocaleString("en-US"), sub: `Forecast ${selectedYearLabel} · Ecommerce` },
    { label: "New Customers", value: Math.round(newCustomersForYear(selectedYearKey) || 0).toLocaleString("en-US"), sub: `Forecast ${selectedYearLabel} · Unique new customers` },
    { label: "ROAS", value: `${roasForYear(selectedYearKey).toFixed(1)}x`, sub: `Forecast ${selectedYearLabel} · Scenario assumption` },
    { label: "Ad Spend", value: formatFinancialMoney(bridges[selectedYearKey].adSpend, {dashZero:true}), sub: `Forecast ${selectedYearLabel} · Advertising` },
    { label: "Net / Gross Ratio", value: formatPercent(b.grossSales ? b.netSales / b.grossSales : 0), sub: `Forecast ${selectedYearLabel} · Net Sales / Gross Sales` },
    { label: "Checkout Abandonment Rate", value: "—", sub: `Forecast ${selectedYearLabel} · Shopify KPI` }
...
function cashFlowRows(yearKey) {
  ...
  const fundingRow = selectedFundingRow();
  // Header Funding Date is the active model trigger; scenario row date is only the default/fallback.
  const fundingDate = String((STATE.meta && STATE.meta.fundingDate) || fundingRow.date || "");
...
function renderCommercialCashFlow() {
  ...
  // Cash Summary must follow the same Display Year as Tabs 2, 3 and 5.
  const selectedCashYearKey = `y${String((STATE.meta && STATE.meta.displayYear) || "2026")}`;
  const selectedCashYearLabel = yearLabel(selectedCashYearKey);
  const selectedFlow = flow[selectedCashYearKey] || flow.y2026;
  const selectedTotals = totals[selectedCashYearKey] || totals.y2026;
  const selectedOperatingCashOut = Number(selectedFlow.operatingCashOut || 0);
  const cashCoverage = selectedOperatingCashOut > 0
    ? `${Math.max(0, (selectedTotals.ending / (selectedOperatingCashOut / 12))).toFixed(1)} mo`
    : "—";
  const minimumBuffer = Number((STATE.cashFlow && STATE.cashFlow.minimumCashBuffer) || 0);
  const capex = selectedFlow.cashOut["CapEx"] || 0;
  const endingDelta = selectedTotals.ending - selectedTotals.opening;
  const endingDeltaPct = selectedTotals.opening ? endingDelta / selectedTotals.opening : 0;
  const lastUpdated = (STATE.meta && STATE.meta.lastUpdated) || new Date().toISOString().slice(0, 10);
  const cashRows = [
    { label: "Opening Cash", value: selectedTotals.opening, icon: "wallet", tone: "neutral" },
    { label: "Cash In", value: selectedTotals.cashIn, icon: "in", tone: "positive" },
    { label: "Funding", value: selectedFlow.cashIn["Funding"] || 0, icon: "bank", tone: "positive" },
    { label: "Operating Cash Out", value: selectedTotals.cashOut, icon: "out", tone: "neutral" },
    ...
    el("div", { class: "cash-ui-head" }, [
      el("div", { class: "cash-title-wrap" }, [
        ...
        el("div", {}, [
          ...
          el("p", { class: "cash-subtitle" }, `Commercial Cash Flow · ${selectedCashYearLabel} executive view`)
        ])
      ]),
      ...
      el("div", { class: "cash-hero compact" }, [
        el("div", { class: "cash-hero-top" }, [
          ...
          el("div", {}, [
            ...
            el("div", { class: moneyClass(formatFinancialMoney(selectedTotals.ending, {dashZero:true}), "cash-hero-value") }, formatFinancialMoney(selectedTotals.ending, {dashZero:true}))
          ])
        ]),
        ...
        el("div", { class: "cash-mini-grid" }, [
          el("div", { class: "cash-mini-pill" }, [el("span", {}, "Cash In"), el("strong", {}, formatFinancialMoney(selectedTotals.cashIn, {dashZero:true}))]),
          el("div", { class: "cash-mini-pill" }, [el("span", {}, "Cash Out"), el("strong", {}, formatFinancialMoney(selectedTotals.cashOut, {dashZero:true}))]),
          el("div", { class: "cash-mini-pill" }, [el("span", {}, "Funding"), el("strong", {}, formatFinancialMoney(selectedFlow.cashIn["Funding"] || 0, {dashZero:true}))]),
...
function adSpendActuals(rows, kpiActuals) {
  ...
  const throughMonth = Math.min(kpiActuals.latest.month, (STATE && STATE.meta && STATE.meta.actualsThroughMonth) || kpiActuals.latest.month);
  const ytd = rowsForYtd(rows, kpiActuals.latest.year, throughMonth);
  const spend = sumField(ytd, "ad_spend") || sumField(ytd, "spend");
  const purchases = sumField(ytd, "purchases") || sumField(ytd, "attributed_purchases") || sumField(ytd, "nb_purchases");
  const weightedRoasNumerator = (ytd || []).reduce((sum, r) => sum + (parseNumber(r.ad_spend || r.spend) * parseNumber(r.roas)), 0);
  const roas = spend ? weightedRoasNumerator / spend : 0;
  const cos = kpiActuals.grossSales ? spend / kpiActuals.grossSales : 0;
  const cac = purchases ? spend / purchases : (kpiActuals.newCustomers ? spend / kpiActuals.newCustomers : 0);
  return { spend, purchases, roas, cos, cac, cacSource: purchases ? "Attributed Purchases" : "New Customers fallback" };
}
...
function revenueCarryoverActuals(rows, latest) {
  ...
  const ytd = rowsForYtd(rows, latest.year, throughMonth);
  const explicitReturning = sumField(ytd, "returning_revenue") || sumField(ytd, "revenue_returning");
  const explicitTotal = sumField(ytd, "total_revenue") || sumField(ytd, "total_customer_revenue");
  if (explicitTotal > 0) return explicitReturning / explicitTotal;

  let returningRevenue = 0;
  let totalRevenue = 0;
  ytd.forEach(row => {
    const type = String(row.customer_type || row.type || row.segment || row.customer_status || "").toLowerCase();
    const revenue = parseNumber(row.revenue || row.net_sales || row.gross_sales || row.amount || row.total_sales);
    totalRevenue += revenue;
    if (type.includes("return") || type.includes("repeat")) returningRevenue += revenue;
  });
  return totalRevenue > 0 ? returningRevenue / totalRevenue : null;
}
...
function applyActualsToState(corroBundle, cavaliBundle) {
  ...
  if (corro) {
    ...
    if (retention) {
      ...
      const carryoverActual = revenueCarryoverActuals(corroBundle.newVsReturning, corro.latest);
      if (carryoverActual !== null) {
        setCurrentInRows(retention.rows, "Incremental Revenue Carryover %", formatPercent(carryoverActual));
        setYearInRows(retention.rows, "Incremental Revenue Carryover %", "y2026", formatPercent(carryoverActual));
      }
    ...
    if (ecommerce) {
      ...
      const ecommerceGm = ecommerceMetrics.gm1 || corro.gm1;
      setCurrentInRows(ecommerce.rows, "GM1 %", formatPercent(ecommerceGm));
      const month = Math.max(1, corro.latest.month);
      const ordersForecast = ecommerceOrders + (ecommerceOrders / month) * Math.max(0, 12 - month);
      setYearInRows(ecommerce.rows, "Orders", "y2026", Math.round(ordersForecast).toLocaleString("en-US"));
      setYearInRows(ecommerce.rows, "AOV", "y2026", formatMoney(ecommerceAov));
      setYearInRows(ecommerce.rows, "GM1 %", "y2026", formatPercent(ecommerceGm));
    }
    if (concierge) {
      ...
      setYearInRows(concierge.rows, "GM1 %", "y2026", conciergeGm ? formatPercent(conciergeGm) : "—");
    }
    if (wellington) {
      ...
      setYearInRows(wellington.rows, "AOV", "y2026", wellingtonMetrics.aov ? formatMoney(wellingtonMetrics.aov) : "—");
      setYearInRows(wellington.rows, "GM1 %", "y2026", wellingtonGm ? formatPercent(wellingtonGm) : "—");
    }
...
function channelMetricsYtd(rows, channel, latest) {
  if (!rows || !rows.length || !latest) return { grossSales:0, netSales:0, cogs:0, grossProfit:0, gm1:0, orders:0, units:0, customers:0, aov:0 };
  const throughMonth = Math.min(latest.month, (STATE && STATE.meta && STATE.meta.actualsThroughMonth) || latest.month);
  const ytd = rowsForYtd(rows, latest.year, throughMonth)
    .filter(r => String(r.channel || "").toLowerCase().includes(String(channel).toLowerCase()));
  const grossSales = sumField(ytd, "gross_sales") || sumField(ytd, "amount");
  const netSales = sumField(ytd, "net_sales") || grossSales;
  const cogs = sumField(ytd, "cogs");
  const grossProfit = sumField(ytd, "gross_profit") || (netSales && cogs ? netSales - cogs : 0);
  const orders = sumField(ytd, "nb_orders");
  const units = sumField(ytd, "nb_units");
  const customers = sumField(ytd, "unique_customers");
  const gm1 = netSales ? grossProfit / netSales : 0;
  return { grossSales, netSales, cogs, grossProfit, gm1, orders, units, customers, aov: orders ? grossSales / orders : 0 };
}
...
function setCavaliForecastFields(cavaliEngine, cavali, cavaliAds) {
  ...
  const rows = cavaliEngine.rows;
  const seedIfBlank = (driver, year, value) => {
    const row = getRow(rows, driver);
    if (row && isBlankLike(row[year])) row[year] = value;
  };

  const month = Math.max(1, Number((STATE.actuals || {}).actualsThroughMonth || STATE.meta.actualsThroughMonth || 12));
  const cavaliOrdersForecast = cavali && cavali.orders ? cavali.orders + (cavali.orders / month) * Math.max(0, 12 - month) : 0;
  seedIfBlank("Orders", "y2026", cavaliOrdersForecast ? Math.round(cavaliOrdersForecast).toLocaleString("en-US") : "—");
  ["Signature Active Members", "Signature Boxes per Year", "Signature Price",
   "Premium Active Members", "Premium Boxes per Year", "Premium Price", "GM1 %"]
    .forEach(driver => {
      const row = getRow(rows, driver);
  ...
  if (!roasRow.driver) {
    rows.push({ driver: "Cavali ROAS", current: "—", y2026: STATE.meta.roas || "4.0x", y2027: STATE.meta.roas || "4.0x", y2028: STATE.meta.roas || "4.0x", y2029: STATE.meta.roas || "4.0x", note: "Paid growth revenue = Cavali Ad Spend × Cavali ROAS." });
  }

  // Actual ad data seeds 2026 only. Future years stay user-editable and are never overwritten by Refresh Actuals.
  if (cavaliAds && cavaliAds.spend) seedIfBlank("Cavali Ad Spend", "y2026", formatMoney(cavaliAds.spend));
  if (cavaliAds && cavaliAds.cac) seedIfBlank("Cavali CAC", "y2026", formatMoney(cavaliAds.cac));
}
...
async function fetchConnectedActualsJson() {
  try {
    const response = await fetch(`data/connected_actuals.json?ts=${Date.now()}`, { cache: "no-store" });
    if (!response.ok) return null;
    const json = await response.json();
    return json && json.brands ? json : null;
  } catch (error) {
    console.warn("Connected actuals JSON not available", error);
    return null;
  }
}

function connectedBrandBundle(json, brand) {
  const data = json && json.brands ? json.brands[brand] : null;
  if (!data) return null;
  return {
    kpis: data.kpis_daily || [],
    revenueShare: data.revenue_share || [],
    newVsReturning: data.new_vs_returning || [],
    adSpend: data.ad_spend || [],
    smartrrProductVolume: data.smartrr_product_volume || data.smartrr_subscribers || [],
    productsQ1: data.products_q1_2026 || [],
...
async function refreshActualsFromSheets({ silent = false } = {}) {
  ...
  try {
    ...
    const [shopifyJson, connectedJson] = await Promise.all([fetchShopifyActualsJson(), fetchConnectedActualsJson()]);
    let baseCorroBundle = connectedBrandBundle(connectedJson, "corro") || { kpis: [], revenueShare: [], newVsReturning: [], adSpend: [], smartrrProductVolume: [], productsQ1: [] };
    let baseCavaliBundle = connectedBrandBundle(connectedJson, "cavali") || { kpis: [], revenueShare: [], newVsReturning: [], adSpend: [], smartrrProductVolume: [], productsQ1: [] };
    ...
    if (!connectedJson && corroSource && cavaliSource) {
      try {
        ...
        console.warn("Google Sheets support data unavailable. Continuing with Shopify JSON if present.", sheetErr);
